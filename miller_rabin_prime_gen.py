...
label1 = tk.Label(wind, text='Génération des nombres premiers')
label1.config(font=('times', 18, 'bold'))
canvas1.create_window(400, 25, window=label1)

label_a = tk.Label(wind, text='Bits :')
...
#---------------------------------------------------------------
# generation de nombres premiers en utilisant le test de Miller-Rabin
#---------------------------------------------------------------
def gen_prime():
    bits = int(input_a.get())
    while True:
        n = random.randrange(2**(bits-1), 2**bits)
        if MillerRabin(n):
            label3 = tk.Label(wind,text=str(n),font=('Lucida Console', 25, 'bold'))
            print(n)
            return canvas1.create_window(400, 450, window=label3)
    

if __name__ == '__main__':
    button1 = tk.Button(text='Générer', command=gen_prime, bg='#fbb034', fg='white', font=('Lucida Console', 18, 'bold'))
